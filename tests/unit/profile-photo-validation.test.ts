...
import { PROFILE_PHOTO_MAX_SIZE_BYTES } from "@/features/profile-photo/profile-photo.types";
import {
  validateProfilePhoto,
  type ProfilePhotoFileLike,
} from "@/features/profile-photo/profile-photo-validation";

function createFile(overrides: Partial<ProfilePhotoFileLike> = {}): ProfilePhotoFileLike {
  return {
    size: 1024,
    type: "image/jpeg",
...
describe("validateProfilePhoto", () => {
  ...
  it("rejects an empty file", async () => {
    await expect(validateProfilePhoto(createFile({ size: 0 }))).rejects.toMatchObject({
      code: "EMPTY_FILE",
    });
  });
  ...
  it("rejects a file larger than 5 MB", async () => {
    await expect(
      validateProfilePhoto(createFile({ size: PROFILE_PHOTO_MAX_SIZE_BYTES + 1 })),
  ...
  it("rejects unsupported content types", async () => {
    await expect(validateProfilePhoto(createFile({ type: "image/gif" }))).rejects.toMatchObject({
