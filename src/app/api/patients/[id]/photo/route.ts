...
export async function GET(
  ...
  if (!session) {
    return toJsonResponse(
      fail("UNAUTHENTICATED", "Not authenticated"),
      401,
    );
  }
  ...
  try {
    ...
    const photo = await profilePhotoService.download(
      "patients",
      id,
    );
...
export async function POST(
  ...
  try {
    ...
    const result = await profilePhotoService.replace(
      "patients",
      id,
      photo,
    );
    ...
    return toJsonResponse(
      ok(toProfilePhotoMutationData(result)),
    );
  } catch (error) {
    return toProfilePhotoErrorResponse(error);
  }
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    const result = await profilePhotoService.remove(
      "patients",
      id,
    );
